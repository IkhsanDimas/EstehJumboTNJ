...
<div x-data="menuFilter({{ $filteredProducts->count() }})" class="bg-page-soft text-slate-800 relative overflow-hidden">
    ...
    ════════════════════════════════════════════════════ --}}
    <section class="relative bg-gradient-to-b from-emerald-100/40 via-emerald-50/20 to-transparent border-b border-emerald-100/10 pt-28 pb-14">
                ...
                <form id="search" action="{{ route('menu') }}" method="GET" class="md:w-96 w-full scroll-mt-32">
                    ...
                    <label class="relative block">
                        ...
                        <input type="search"
                               ...
                               autocomplete="off"
                               class="w-full bg-white border border-slate-200 rounded-full pl-11 pr-12 py-3.5 text-sm text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-100 focus:border-emerald-500 transition">
                        <button type="button"
    ...
    ════════════════════════════════════════════════════ --}}
    <div class="sticky top-0 z-30 bg-white/85 backdrop-blur-md border-b border-emerald-100/20 shadow-[0_4px_20px_-12px_rgba(16,185,129,0.15)]">
        <div class="max-w-7xl mx-auto px-6">
                ...
                <a href="{{ route('menu', $query !== '' ? ['q' => $query] : []) }}"
                   class="flex-shrink-0 px-4 py-2 rounded-full text-sm font-semibold transition
                          {{ $activeCategory === 'all'
                                ...
                                : 'bg-white text-slate-600 hover:bg-emerald-50 hover:text-emerald-700 border border-slate-150' }}">
                    Semua
    ...
    <section class="relative max-w-7xl mx-auto px-6 py-16">
        @if ($filteredProducts->count() > 0)
            ...
            <div x-show="visibleCount === 0 && filtering" x-cloak
                ...
                <button type="button" @click="clearSearch()" class="mt-5 inline-flex items-center gap-1.5 text-sm font-semibold text-emerald-600 hover:text-emerald-700">
                    Bersihkan pencarian →
                </button>
            </div>
        @else
            <div class="bg-slate-50 border border-dashed border-slate-200 rounded-2xl p-14 text-center">
                ...
                <a href="{{ route('menu') }}" class="mt-5 inline-flex items-center gap-1.5 text-sm font-semibold text-emerald-600 hover:text-emerald-700">
                    Lihat semua menu →
                </a>
            </div>
        @endif
    ...
    <section class="relative max-w-7xl mx-auto px-6 pb-24">
        <div class="relative rounded-3xl overflow-hidden bg-gradient-to-br from-slate-900 via-slate-900 to-emerald-950 px-8 py-10 md:px-12 md:py-12 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div aria-hidden="true" class="absolute -top-16 -right-16 w-72 h-72 rounded-full bg-emerald-500/20 blur-3xl"></div>
            ...
            <a href="https://wa.me/{{ config('services.whatsapp.number') }}" target="_blank" rel="noopener"
               class="relative inline-flex items-center gap-2 bg-white text-slate-900 hover:bg-emerald-50 text-sm font-semibold px-6 py-3.5 rounded-full transition shadow-lg w-fit">
                Chat Sekarang
